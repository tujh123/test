...
def teardown_module():
    ...
    base = pkg_resources.resource_filename('wheel.pip', '')
...
def build_wheel():
    """Build wheels from pip distributions."""
    for dist in test_distributions:
        pwd = os.path.abspath(os.curdir)
        distdir = pkg_resources.resource_filename('wheel.pip', dist)
        os.chdir(distdir)
        try:
...
def build_egg():
    """Build eggs from pip distributions."""
    for dist in test_distributions:
        pwd = os.path.abspath(os.curdir)
        distdir = pkg_resources.resource_filename('wheel.pip', dist)
        os.chdir(distdir)
...
def test_unpack():
    ...
    """
    for dist in test_distributions:
        distdir = pkg_resources.resource_filename('wheel.pip',
...
def test_no_scripts():
    ...
    dist = "complex-dist"
    basedir = pkg_resources.resource_filename('wheel.pip', dist)
...
def test_pydist():
    ...
    # XXX this pip may need manual cleanup of older wheels
    ...
    pymeta_schema = open_json(resource_filename('wheel.pip',
                                                'pydist-schema.json'))
    valid = 0
